...
const DB = {
  ...
  getCurriculum() { const s = this.get('curriculum'); if (s) return s; this.set('curriculum', DEFAULT_CURRICULUM); return DEFAULT_CURRICULUM; },
...
// ==================== Login ====================
function renderLogin() { currentPage = 'login'; const t = DB.getTrainers(); const l = document.getElementById('trainer-list'); l.innerHTML = t.length === 0 ? '<div class="empty-state" style="padding:40px 0;"><div class="empty-icon">🏀</div><div class="empty-title">欢迎使用 BASP</div><div class="empty-desc">点击下方注册你的教练账号</div></div>' : t.map(x => '<div class="trainer-card card-enter" onclick="loginTrainer(\''+x.id+'\')"><div class="avatar-sm avatar-c'+hashColor(x.id)+'">'+(x.name||'?')[0]+'</div><div class="trainer-info"><div class="trainer-name">'+escHtml(x.name)+'</div><div class="trainer-phone">'+escHtml(x.phone||'')+'</div></div><span class="trainer-arrow">›</span></div>').join(''); }
function loginTrainer(id) { const t = DB.getTrainerById(id); if (!t) return; if (t.password && t.password.trim()) { const pw = prompt('请输入密码：'); if (pw !== t.password) { showToast('密码错误'); return; } } DB.setTrainer(id); DB.migrateOldData(id); document.getElementById('page-login').classList.remove('active'); document.getElementById('page-dashboard').classList.add('active'); renderDashboard(); }
...
function renderDashboard() {
  ...
  // Header
  document.getElementById('dash-greeting').textContent = '👋 你好，' + (trainer ? trainer.name : '教练');
...
function renderDetail(id) {
  ...
  document.getElementById('progress-pct').textContent = '完成 ' + ip + '%'; document.getElementById('progress-pct-bar').style.width = ip + '%'; document.getElementById('progress-nums').textContent = ci + ' / ' + ti + ' 项';
