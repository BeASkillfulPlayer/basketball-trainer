...
function renderDetail(id) {
  ...
  document.getElementById('record-list').innerHTML = records.length === 0 ? '' : records.map(r => '<div class="record-card"><div class="record-header"><span class="record-date">'+r.date+'</span><span class="record-dur">'+r.duration+'课时</span><span class="record-del" onclick="deleteRecord(event,\''+r.id+'\')"></span></div>'+(r.checkedItems&&r.checkedItems.length>0||r.customItems&&r.customItems.length>0?'<div class="record-tags">'+(r.checkedItems||[]).filter(function(i){return i;}).map(function(i){return '<span class="tag">'+escHtml(i)+'</span>';}).join('')+(r.customItems||[]).filter(function(i){return i;}).map(function(i){return '<span class="tag tag-custom">'+escHtml(i)+'</span>';}).join('')+'</div>':'<div class="record-no-items">未记录训练项目</div>')+(r.notes?'<div class="record-notes-text">'+escHtml(r.notes)+'</div>':'')+'</div>').join('');
