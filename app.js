...
// ==================== Login ====================
function renderLogin() { currentPage = 'login'; cancelAnimationFrame(window._loginFrame); var t = DB.getTrainers(); var l = document.getElementById('trainer-list'); l.innerHTML = t.length === 0 ? '<div style="padding:20px 0;"><button class="btn btn-primary" onclick="showRegister()" style="background:var(--red);">创建新账号</button></div>' : t.map(function(x){ var av = x.avatar ? '<img src="'+x.avatar+'" alt="">' : (x.name||'?')[0]; return '<div class="trainer-card card-enter" onclick="loginTrainer(\''+x.id+'\')"><div class="trainer-avatar avatar-c'+hashColor(x.id)+'">'+av+'</div><div class="trainer-info"><div class="trainer-name">'+escHtml(x.name)+'</div><div class="trainer-phone">'+escHtml(x.phone||'')+'</div></div></div>'; }).join(''); startLoginParticles(); }
